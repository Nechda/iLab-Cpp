...
std::vector<Vertex> vertices = {
    //{{-0.5f, -0.5f,0}, {0.0f, 1.0f, 0.0f} , {0,0,1.0f}},
    //{{0.5f, -0.5f,0}, {0.0f, 1.0f, 0.0f}, {0,0,1.0f}},
    //{{0.5f, 0.5f,0}, {0.0f, 1.0f, 0.0f}, {0,0,1.0f}},
...
void create_triangles(glm::vec3 v[3], glm::vec3 colour) {
    glm::vec3 n = glm::cross(v[0] - v[1], v[0] - v[2]);
    n = glm::normalize(n);

    for(int i = 0; i < 3; i++)
        vertices.push_back({v[i], colour, n});
}
...
void make_scene() {
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        create_triangles(vert, colors[is_intersected]);
    }
}
...
private:
    ...
    void initWindow() {
        ...
        auto& win_info = WindowInfo::win_info;
        win_info.descriptor = window;
    ...
    void mainLoop() {
        ...
        while (!should_clouse) {
            ...
            drawFrame();
            scene_camera.update();
            should_clouse |= glfwWindowShouldClose(window);
            should_clouse |= glfwGetKey(winfo.descriptor, GLFW_KEY_Q) == GLFW_PRESS;
            should_clouse |= glfwGetKey(winfo.descriptor, GLFW_KEY_ESCAPE) == GLFW_PRESS;
        }
